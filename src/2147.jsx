...
/* ============================================================
   Package Labelling Strategy Simulator — Orders A, B and C
...
function buildScenario(n) {
  ...
  if (n === 2) {
    duration = 31;
    keyMessage = "Correct final numbering for every order, but multi-package orders wait in the packing area — the more parcels, the longer the wait.";
    // Waiting bays are placed to the right of the packing area, outside the
    // ORTEC infeed branches. This keeps all conveyor lanes visually clear.
    const SLOTS = [
      [{ x: 7.0, z: 5.0 }],
      [{ x: 8.6, z: 5.0 }, { x: 10.2, z: 5.0 }],
      [{ x: 7.0, z: 6.4 }, { x: 8.6, z: 6.4 }, { x: 10.2, z: 6.4 }],
    ];
  ...
  if (n === 3) {
    duration = 32;
    keyMessage = "Immediate release from all stations with correct numbering downstream — but every incomplete order sends parcels through the waiting loop.";
    const loopFrom = (t0) => [
    ...
    messages.push([0, "3 stations packing — every parcel gets an temporary label and leaves immediately"]);
...
const SCENARIOS = {
  1: { title: "Label immediately", short: "S1" },
  2: { title: "Wait, then label", short: "S2" },
  3: { title: "Temporary label, final label later", short: "S3" },
  4: { title: "Label based on ORTEC proposal", short: "S4" },
};
...
export default function ParcelLabelSim() {
  ...
  useEffect(() => {
    ...
    // Waiting area (scenario 2) — compact side bay outside all conveyor lanes
    const staging = new THREE.Group();
    const waitingBayWidth = 5.4;
    const waitingBayDepth = 3.4;
    const waitingBayX = 8.6;
    const waitingBayZ = 5.7;

    const zone = new THREE.Mesh(
      new THREE.PlaneGeometry(waitingBayWidth, waitingBayDepth),
    ...
    zone.position.set(waitingBayX, 0.02, waitingBayZ);
    staging.add(zone);
    ...
    zoneBorder.position.set(waitingBayX, 0.03, waitingBayZ);
    ...
    [
      ...
      pallet.position.set(x, 0.45, z);
      pallet.castShadow = true;

      const palletLegs = new THREE.Mesh(
        new THREE.BoxGeometry(1.2, 0.35, 1.1),
        mat(0x6f5531, 0.95)
      );
      palletLegs.position.set(x, 0.2, z);
      staging.add(pallet, palletLegs);
    });

    const stagingSign = makeTextPlane(
      "WAITING AREA · UNTIL ORDER COMPLETE",
      C.yellow,
      5.4,
      0.55
    );
    stagingSign.position.set(waitingBayX, 2.2, 7.55);
    ...
    // order banner
    const orderSign = makeTextPlane("PACKING AREA — 1 × 1P · 1 × 2P · 1 × 3P · ONE TO SHIPPING LINE", "#e8edf4", 9.5, 0.7);
  ...
  const setView = (name) => {
    ...
    const views = {
      ...
      "ORTEC Routing": { target: [-2.2, 0.8, 7], theta: -0.6, phi: 0.95, radius: 17 },
      "Waiting Area": { target: [8.6, 0.6, 5.7], theta: -0.65, phi: 0.95, radius: 14 },
      "Label Check": { target: [MACHINE_X, 1.5, 0], theta: -1.2, phi: 1.0, radius: 10 },
      "Waiting Loop": { target: [MACHINE_X - 3.5, 0.5, -2], theta: -2.2, phi: 0.9, radius: 15 },
      "Label Correction": { target: [MACHINE_X + 3, 1.2, 2.2], theta: -0.5, phi: 0.95, radius: 13 },
    };
    const v = views[name];
  ...
  // ---------- comparison table ----------
  const compareRows = [
    ["Package leaves packing immediately", "Yes", "No", "Yes", "Yes"],
    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, height: "100dvh", maxHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Space Grotesk', system-ui, sans-serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ padding: "10px 14px 8px", borderBottom: `1px solid ${C.line}`, display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
        <div style={{ marginRight: "auto" }}>
          <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: 0.3 }}>Multi-Package Order Labelling — Simulator</div>
      ...
      <div style={{ flex: 1, position: "relative", minHeight: 0 }}>
        ...
        <div style={{ position: "absolute", bottom: 10, left: 10, display: "flex", flexDirection: "column", gap: 5 }}>
          {["Overview", "Packing Stations", scenario >= 2 ? "ORTEC Routing" : null, scenario === 2 ? "Waiting Area" : null, scenario === 3 || scenario === 4 ? "Label Check" : null, scenario === 3 ? "Waiting Loop" : null, scenario === 4 ? "Label Correction" : null]
            .filter(Boolean)
            .map((v) => (
        ...
        <div style={{ position: "absolute", top: 56, right: 10, width: 235, background: "rgba(20,27,37,0.94)", border: `1px solid ${C.line}`, borderRadius: 10, fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5, overflow: "hidden" }}>
          ...
          {panelOpen && (
            <div style={{ padding: "6px 10px 10px" }}>
              ...
              {stat("Orders", "A · B · C")}
              {stat("Packages packed", `${hud.packed} / ${nParcels}`, hud.packed === nParcels ? C.green : C.text)}
              {stat("Packages with final label", `${hud.labelled} / ${nParcels}`, hud.labelled === nParcels ? C.green : C.text)}
              {stat("Packages waiting", hud.waiting, hud.waiting ? C.yellow : C.dim)}
              {stat("Packages moving outbound", hud.onConv)}
              {stat(scenario === 4 ? "Packages in label correction" : "Packages in waiting loop", hud.inLoop, hud.inLoop ? (scenario === 4 ? C.red : C.orange) : C.dim)}
              {stat("Avg wait / parcel", `${hud.avgWait.toFixed(1)}s`)}
              {stat("Additional process steps", hud.handling)}
              {stat("Label checks", hud.scans)}
            </div>
          )}
        </div>
        ...
        {showCompare && (
          <div style={{ position: "absolute", inset: 0, background: "rgba(13,18,25,0.92)", overflow: "auto", padding: 16 }}>
            <div style={{ maxWidth: 640, margin: "0 auto" }}>
              <div style={{ fontSize: 18, fontWeight: 700, marginTop: 6 }}>Comparison of Package Labelling Strategies</div>
              <div style={{ fontSize: 12.5, color: C.dim, margin: "6px 0 14px", lineHeight: 1.5 }}>
                The simulation compares immediate package release, waiting until an order is complete, and final labelling later in the process using a temporary package label.
              </div>
